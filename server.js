...
async function api(req, res) {
  ...
  try {
    ...
    if (url.pathname === '/api/bans' && method === 'GET') return send(res, 200, { ok:true, bans: db.bans });

    if (url.pathname === '/api/priority-list' && method === 'GET') return send(res, 200, { ok:true, priorityList: db.priorityList || [] });
